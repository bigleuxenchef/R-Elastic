...
the table as defined in hive looks like 
...
ER_Plot1 <- function(x,chart_title="no title"){
  ...
  if (NROW(x) > 0) hist(x,main= paste("Duration stats Process:",chart_title), xlab="Seconds",cex.main=1)
}
...
ER_Plot3 <- function(p,chart_title="no title"){
  if (NROW(p) > 0) {
    ggplot(NULL,aes(p,y=..count..),cex.main=1) + 
      geom_histogram(aes(y = ..density..)) +
      # aes(fill=..count..)) + 
      scale_y_sqrt() +
      geom_density(aes(y = ..density..),colour="black",linetype=1,size=.2,fill=hcl(100,180,70,.3)) +
